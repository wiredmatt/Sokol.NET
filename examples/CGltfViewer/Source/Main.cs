...
public static unsafe partial class GltfViewer
{
    ...
    class _state
    {
        ...
        // Pending model data — Phase 1: main gltf/glb file bytes
        public string? pendingModelPath = null;
        public byte[]? pendingModelBuffer = null;

        // Pending model data — Phase 2: parsed cgltf_data waiting for external .bin files
        // BeginParse returns the IntPtr to cgltf_data* and the GCHandle for the main buffer.
        public unsafe CGltfModel.ParsedGltf? pendingParsed = null;
        public int pendingExternalBinCount = 0;  // how many external .bin fetches are outstanding
        public int pendingTextureCount = 0;            // how many external texture prefetches are outstanding
        public int pendingTotalAsyncFileCount = 0;     // total .bin + texture fetches started (for progress %)
        public Dictionary<string, byte[]> pendingPreloadedTextures = new();  // texPath→bytes
        ...
        public List<(CGltfNode node, int lightIndex)> lightNodes = new List<(CGltfNode, int)>();
    ...
    public static SApp.sapp_desc sokol_main()
    {
        return new SApp.sapp_desc()
        {
            ...
            height = 0,
            sample_count = 4,
            window_title = "CGltfViewer (sokol-app)",
