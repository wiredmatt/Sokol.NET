...
namespace Sokol
{
    ...
    public static class EnvironmentMapLoader
    {
        ...
        /// Note: cgltf does not support EXT_lights_image_based, so this always returns null
        /// (keeping the existing HDR environment loaded from a standalone file).
        /// </summary>
        /// <param name="model">The loaded cgltf model (can be null)</param>
        /// <param name="name">Name for the environment map</param>
        /// <returns>EnvironmentMap or null if creation failed</returns>
        public static unsafe EnvironmentMap? LoadFromGltfOrCreateTest(CGltfModel? model, string name = "environment")
        {
            // cgltf does not support EXT_lights_image_based — always keep existing HDR environment
            Info("[IBL] cgltf has no EXT_lights_image_based support; keeping existing environment");
            return null;
        }
