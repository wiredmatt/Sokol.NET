...
#define TONEMAP_ACES_NARKOWICZ 1
...
vec3 toneMap(vec3 color)
{
    ...
    if(u_type == TONEMAP_ACES_NARKOWICZ)
    {
        color = toneMapACES_Narkowicz(color);
    }

   if(u_type == TONEMAP_ACES_HILL)
   {
        color = toneMapACES_Hill(color);
   }

   if(u_type == TONEMAP_ACES_HILL_EXPOSURE_BOOST)
   {
   ...
   if(u_type == TONEMAP_KHR_PBR_NEUTRAL)
   {
