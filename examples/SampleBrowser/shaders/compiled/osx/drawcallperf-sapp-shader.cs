using static Sokol.SG;
using static Sokol.SG.sg_backend;
using static Sokol.SG.sg_shader_stage;
using static Sokol.SG.sg_uniform_type;
using static Sokol.SG.sg_uniform_layout;
using static Sokol.SG.sg_image_type;
using static Sokol.SG.sg_sampler_type;
using static Sokol.SG.sg_image_sample_type;
using static Sokol.SG.sg_shader_attr_base_type;
using System.Numerics;
using System.Runtime.InteropServices;


using hmm_vec2=System.Numerics.Vector2;
using hmm_vec3=System.Numerics.Vector3;
using hmm_vec4=System.Numerics.Vector4;
using hmm_mat4=System.Numerics.Matrix4x4;


using vec2_t=System.Numerics.Vector2;
using vec3_t=System.Numerics.Vector3;
using vec4_t=System.Numerics.Vector4;
using mat44_t=System.Numerics.Matrix4x4;


namespace drawcallperf_sapp_shader_cs {

public static unsafe class Shaders 
{
    /*
        #version:1# (machine generated, don't edit!)

        Generated by sokol-shdc (https://github.com/floooh/sokol-tools)

        Cmdline:
            sokol-shdc --input shaders/drawcallperf-sapp.glsl --reflection --output shaders/compiled/osx/drawcallperf-sapp-shader.cs --slang metal_macos -f sokol_csharp

        Overview:
        =========
        Shader program: 'drawcallperf':
            Get shader desc: drawcallperf_shader_desc(sg_query_backend());
            Vertex Shader: vs
            Fragment Shader: fs
            Attributes:
                ATTR_drawcallperf_in_pos => 0
                ATTR_drawcallperf_in_uv => 1
                ATTR_drawcallperf_in_bright => 2
        Bindings:
            Uniform block 'vs_per_frame':
                C struct: vs_per_frame_t
                Bind slot: UB_vs_per_frame => 0
            Uniform block 'vs_per_instance':
                C struct: vs_per_instance_t
                Bind slot: UB_vs_per_instance => 1
            Texture 'tex':
                Image type: SG_IMAGETYPE_2D
                Sample type: SG_IMAGESAMPLETYPE_FLOAT
                Multisampled: false
                Bind slot: VIEW_tex => 0
            Sampler 'smp':
                Type: SG_SAMPLERTYPE_FILTERING
                Bind slot: SMP_smp => 0
    */
    public const int ATTR_drawcallperf_in_pos = 0;
    public const int ATTR_drawcallperf_in_uv = 1;
    public const int ATTR_drawcallperf_in_bright = 2;
    public const int UB_vs_per_frame = 0;
    public const int UB_vs_per_instance = 1;
    public const int VIEW_tex = 0;
    public const int SMP_smp = 0;
    
[StructLayout(LayoutKind.Sequential)]
    public struct vs_per_frame_t {
        public vs_per_frame_t(){}
        public mat44_t viewproj;
    };
    
[StructLayout(LayoutKind.Sequential)]
    public struct vs_per_instance_t {
        public vs_per_instance_t(){}
        public vec4_t world_pos;
    };
    /*
        #include <metal_stdlib>
        #include <simd/simd.h>

        using namespace metal;

        struct vs_per_frame
        {
            float4x4 viewproj;
        };

        struct vs_per_instance
        {
            float4 world_pos;
        };

        struct main0_out
        {
            float2 uv [[user(locn0)]];
            float bright [[user(locn1)]];
            float4 gl_Position [[position]];
        };

        struct main0_in
        {
            float3 in_pos [[attribute(0)]];
            float2 in_uv [[attribute(1)]];
            float in_bright [[attribute(2)]];
        };

        vertex main0_out main0(main0_in in [[stage_in]], constant vs_per_frame& _19 [[buffer(0)]], constant vs_per_instance& _25 [[buffer(1)]])
        {
            main0_out out = {};
            out.gl_Position = _19.viewproj * (_25.world_pos + float4(in.in_pos * 0.0500000007450580596923828125, 1.0));
            out.uv = in.in_uv;
            out.bright = in.in_bright;
            return out;
        }

    */
    static readonly byte[] vs_source_metal_macos = {
            ,    
};
    /*
        #include <metal_stdlib>
        #include <simd/simd.h>

        using namespace metal;

        struct main0_out
        {
            float4 frag_color [[color(0)]];
        };

        struct main0_in
        {
            float2 uv [[user(locn0)]];
            float bright [[user(locn1)]];
        };

        fragment main0_out main0(main0_in in [[stage_in]], texture2d<float> tex [[texture(0)]], sampler smp [[sampler(0)]])
        {
            main0_out out = {};
            out.frag_color = float4(tex.sample(smp, in.uv).xyz * in.bright, 1.0);
            return out;
        }

    */
    static readonly byte[] fs_source_metal_macos = {
            ,    
};
    public static unsafe sg_shader_desc drawcallperf_shader_desc(sg_backend backend) {
        if (backend == SG_BACKEND_METAL_MACOS) {
            sg_shader_desc desc = default(sg_shader_desc);
            desc.vertex_func.source = System.Text.Encoding.UTF8.GetString(vs_source_metal_macos);
            desc.vertex_func.entry = "main0";
            desc.fragment_func.source = System.Text.Encoding.UTF8.GetString(fs_source_metal_macos);
            desc.fragment_func.entry = "main0";
            desc.attrs[0].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[1].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[2].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.uniform_blocks[0].stage = SG_SHADERSTAGE_VERTEX;
            desc.uniform_blocks[0].layout = SG_UNIFORMLAYOUT_STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].msl_buffer_n = 0;
            desc.uniform_blocks[1].stage = SG_SHADERSTAGE_VERTEX;
            desc.uniform_blocks[1].layout = SG_UNIFORMLAYOUT_STD140;
            desc.uniform_blocks[1].size = 16;
            desc.uniform_blocks[1].msl_buffer_n = 1;
            desc.views[0].texture.stage = SG_SHADERSTAGE_FRAGMENT;
            desc.views[0].texture.image_type = SG_IMAGETYPE_2D;
            desc.views[0].texture.sample_type = SG_IMAGESAMPLETYPE_FLOAT;
            desc.views[0].texture.multisampled = false;
            desc.views[0].texture.msl_texture_n = 0;
            desc.samplers[0].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.samplers[0].sampler_type = SG_SAMPLERTYPE_FILTERING;
            desc.samplers[0].msl_sampler_n = 0;
            desc.texture_sampler_pairs[0].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.texture_sampler_pairs[0].view_slot = 0;
            desc.texture_sampler_pairs[0].sampler_slot = 0;
            desc.label = "drawcallperf_shader";
            return desc;
        }
        return default(sg_shader_desc);
    }
    public static int drawcallperf_attr_slot(string attr_name) {
        if (attr_name == "in_pos") {
            return 0;
        }
        if (attr_name == "in_uv") {
            return 1;
        }
        if (attr_name == "in_bright") {
            return 2;
        }
        return -1;
    }
    public static int drawcallperf_texture_slot(string tex_name) {
        if (tex_name == "tex") {
            return 0;
        }
        return -1;
    }
    public static int drawcallperf_sampler_slot(string smp_name) {
        if (smp_name == "smp") {
            return 0;
        }
        return -1;
    }
    public static int drawcallperf_uniformblock_slot(string ub_name) {
        if (ub_name == "vs_per_frame") {
            return 0;
        }
        if (ub_name == "vs_per_instance") {
            return 1;
        }
        return -1;
    }
    public static int drawcallperf_uniformblock_size(string ub_name) {
        if (ub_name == "vs_per_frame") {
            return Marshal.SizeOf<vs_per_frame_t>();
        }
        if (ub_name == "vs_per_instance") {
            return Marshal.SizeOf<vs_per_instance_t>();
        }
        return 0;
    }
    public static int drawcallperf_uniform_offset(string ub_name, string u_name) {
        if (ub_name == "vs_per_frame") {
            if (u_name == "viewproj") {
                return 0;
            }
        }
        if (ub_name == "vs_per_instance") {
            if (u_name == "world_pos") {
                return 0;
            }
        }
        return -1;
    }
    public static sg_glsl_shader_uniform drawcallperf_uniform_desc(string ub_name, string u_name) {
        sg_glsl_shader_uniform res = default;
        if (ub_name == "vs_per_frame") {
            if (u_name == "viewproj") {
                res.type = SG_UNIFORMTYPE_MAT4;
                res.array_count = 0;
                res.glsl_name = "viewproj";
                return res;
            }
        }
        if (ub_name == "vs_per_instance") {
            if (u_name == "world_pos") {
                res.type = SG_UNIFORMTYPE_FLOAT4;
                res.array_count = 0;
                res.glsl_name = "world_pos";
                return res;
            }
        }
        return res;
    }
    public static int drawcallperf_storagebuffer_slot(string sbuf_name) {
        return -1;
    }
    public static int drawcallperf_storageimage_slot(string simg_name) {
        return -1;
    }
}

} // namescape 

