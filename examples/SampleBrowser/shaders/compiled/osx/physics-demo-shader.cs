using static Sokol.SG;
using static Sokol.SG.sg_backend;
using static Sokol.SG.sg_shader_stage;
using static Sokol.SG.sg_uniform_type;
using static Sokol.SG.sg_uniform_layout;
using static Sokol.SG.sg_image_type;
using static Sokol.SG.sg_sampler_type;
using static Sokol.SG.sg_image_sample_type;
using static Sokol.SG.sg_shader_attr_base_type;
using System.Numerics;
using System.Runtime.InteropServices;


using hmm_vec2=System.Numerics.Vector2;
using hmm_vec3=System.Numerics.Vector3;
using hmm_vec4=System.Numerics.Vector4;
using hmm_mat4=System.Numerics.Matrix4x4;


using vec2_t=System.Numerics.Vector2;
using vec3_t=System.Numerics.Vector3;
using vec4_t=System.Numerics.Vector4;
using mat44_t=System.Numerics.Matrix4x4;


namespace physics_demo_shader_cs {

public static unsafe class Shaders 
{
    /*
        #version:1# (machine generated, don't edit!)

        Generated by sokol-shdc (https://github.com/floooh/sokol-tools)

        Cmdline:
            sokol-shdc --input shaders/physics-demo.glsl --reflection --output shaders/compiled/osx/physics-demo-shader.cs --slang metal_macos -f sokol_csharp

        Overview:
        =========
        Shader program: 'physics_demo_smooth':
            Get shader desc: physics_demo_smooth_shader_desc(sg_query_backend());
            Vertex Shader: vs_smooth
            Fragment Shader: fs_smooth
            Attributes:
                ATTR_physics_demo_smooth_position => 0
                ATTR_physics_demo_smooth_normal => 1
                ATTR_physics_demo_smooth_inst_model_0 => 2
                ATTR_physics_demo_smooth_inst_model_1 => 3
                ATTR_physics_demo_smooth_inst_model_2 => 4
                ATTR_physics_demo_smooth_inst_model_3 => 5
                ATTR_physics_demo_smooth_inst_color => 6
        Bindings:
            Uniform block 'vs_params':
                C struct: vs_params_t
                Bind slot: UB_vs_params => 0
            Uniform block 'fs_params':
                C struct: fs_params_t
                Bind slot: UB_fs_params => 1
    */
    public const int ATTR_physics_demo_smooth_position = 0;
    public const int ATTR_physics_demo_smooth_normal = 1;
    public const int ATTR_physics_demo_smooth_inst_model_0 = 2;
    public const int ATTR_physics_demo_smooth_inst_model_1 = 3;
    public const int ATTR_physics_demo_smooth_inst_model_2 = 4;
    public const int ATTR_physics_demo_smooth_inst_model_3 = 5;
    public const int ATTR_physics_demo_smooth_inst_color = 6;
    public const int UB_vs_params = 0;
    public const int UB_fs_params = 1;
    
[StructLayout(LayoutKind.Sequential)]
    public struct vs_params_t {
        public vs_params_t(){}
        public mat44_t vp;
    };
    
[StructLayout(LayoutKind.Sequential)]
    public struct fs_params_t {
        public fs_params_t(){}
        public vec3_t light_dir;
        fixed byte _pad_12[4];
        public vec3_t view_pos;
        fixed byte _pad_28[4];
    };
    /*
        #pragma clang diagnostic ignored "-Wmissing-prototypes"

        #include <metal_stdlib>
        #include <simd/simd.h>

        using namespace metal;

        // Returns the determinant of a 2x2 matrix.
        static inline __attribute__((always_inline))
        float spvDet2x2(float a1, float a2, float b1, float b2)
        {
            return a1 * b2 - b1 * a2;
        }

        // Returns the determinant of a 3x3 matrix.
        static inline __attribute__((always_inline))
        float spvDet3x3(float a1, float a2, float a3, float b1, float b2, float b3, float c1, float c2, float c3)
        {
            return a1 * spvDet2x2(b2, b3, c2, c3) - b1 * spvDet2x2(a2, a3, c2, c3) + c1 * spvDet2x2(a2, a3, b2, b3);
        }

        // Returns the inverse of a matrix, by using the algorithm of calculating the classical
        // adjoint and dividing by the determinant. The contents of the matrix are changed.
        static inline __attribute__((always_inline))
        float4x4 spvInverse4x4(float4x4 m)
        {
            float4x4 adj; // The adjoint matrix (inverse after dividing by determinant)

            // Create the transpose of the cofactors, as the classical adjoint of the matrix.
            adj[0][0] =  spvDet3x3(m[1][1], m[1][2], m[1][3], m[2][1], m[2][2], m[2][3], m[3][1], m[3][2], m[3][3]);
            adj[0][1] = -spvDet3x3(m[0][1], m[0][2], m[0][3], m[2][1], m[2][2], m[2][3], m[3][1], m[3][2], m[3][3]);
            adj[0][2] =  spvDet3x3(m[0][1], m[0][2], m[0][3], m[1][1], m[1][2], m[1][3], m[3][1], m[3][2], m[3][3]);
            adj[0][3] = -spvDet3x3(m[0][1], m[0][2], m[0][3], m[1][1], m[1][2], m[1][3], m[2][1], m[2][2], m[2][3]);

            adj[1][0] = -spvDet3x3(m[1][0], m[1][2], m[1][3], m[2][0], m[2][2], m[2][3], m[3][0], m[3][2], m[3][3]);
            adj[1][1] =  spvDet3x3(m[0][0], m[0][2], m[0][3], m[2][0], m[2][2], m[2][3], m[3][0], m[3][2], m[3][3]);
            adj[1][2] = -spvDet3x3(m[0][0], m[0][2], m[0][3], m[1][0], m[1][2], m[1][3], m[3][0], m[3][2], m[3][3]);
            adj[1][3] =  spvDet3x3(m[0][0], m[0][2], m[0][3], m[1][0], m[1][2], m[1][3], m[2][0], m[2][2], m[2][3]);

            adj[2][0] =  spvDet3x3(m[1][0], m[1][1], m[1][3], m[2][0], m[2][1], m[2][3], m[3][0], m[3][1], m[3][3]);
            adj[2][1] = -spvDet3x3(m[0][0], m[0][1], m[0][3], m[2][0], m[2][1], m[2][3], m[3][0], m[3][1], m[3][3]);
            adj[2][2] =  spvDet3x3(m[0][0], m[0][1], m[0][3], m[1][0], m[1][1], m[1][3], m[3][0], m[3][1], m[3][3]);
            adj[2][3] = -spvDet3x3(m[0][0], m[0][1], m[0][3], m[1][0], m[1][1], m[1][3], m[2][0], m[2][1], m[2][3]);

            adj[3][0] = -spvDet3x3(m[1][0], m[1][1], m[1][2], m[2][0], m[2][1], m[2][2], m[3][0], m[3][1], m[3][2]);
            adj[3][1] =  spvDet3x3(m[0][0], m[0][1], m[0][2], m[2][0], m[2][1], m[2][2], m[3][0], m[3][1], m[3][2]);
            adj[3][2] = -spvDet3x3(m[0][0], m[0][1], m[0][2], m[1][0], m[1][1], m[1][2], m[3][0], m[3][1], m[3][2]);
            adj[3][3] =  spvDet3x3(m[0][0], m[0][1], m[0][2], m[1][0], m[1][1], m[1][2], m[2][0], m[2][1], m[2][2]);

            // Calculate the determinant as a combination of the cofactors of the first row.
            float det = (adj[0][0] * m[0][0]) + (adj[0][1] * m[1][0]) + (adj[0][2] * m[2][0]) + (adj[0][3] * m[3][0]);

            // Divide the classical adjoint matrix by the determinant.
            // If determinant is zero, matrix is not invertable, so leave it unchanged.
            return (det != 0.0f) ? (adj * (1.0f / det)) : m;
        }

        struct vs_params
        {
            float4x4 vp;
        };

        struct main0_out
        {
            float3 world_normal [[user(locn0)]];
            float3 world_pos [[user(locn1)]];
            float3 color [[user(locn2)]];
            float4 gl_Position [[position]];
        };

        struct main0_in
        {
            float3 position [[attribute(0)]];
            float3 normal [[attribute(1)]];
            float4 inst_model_0 [[attribute(2)]];
            float4 inst_model_1 [[attribute(3)]];
            float4 inst_model_2 [[attribute(4)]];
            float4 inst_model_3 [[attribute(5)]];
            float3 inst_color [[attribute(6)]];
        };

        vertex main0_out main0(main0_in in [[stage_in]], constant vs_params& _65 [[buffer(0)]])
        {
            main0_out out = {};
            float4x4 _42 = float4x4(in.inst_model_0, in.inst_model_1, in.inst_model_2, in.inst_model_3);
            float4 _54 = _42 * float4(in.position, 1.0);
            out.gl_Position = _65.vp * _54;
            float4x4 _78 = transpose(spvInverse4x4(_42));
            out.world_normal = fast::normalize(float3x3(_78[0].xyz, _78[1].xyz, _78[2].xyz) * in.normal);
            out.world_pos = _54.xyz;
            out.color = in.inst_color;
            return out;
        }

    */
    static readonly byte[] vs_smooth_source_metal_macos = {
            ,    
    
};
    /*
        #include <metal_stdlib>
        #include <simd/simd.h>

        using namespace metal;

        struct fs_params
        {
            float3 light_dir;
            float3 view_pos;
        };

        struct main0_out
        {
            float4 frag_color [[color(0)]];
        };

        struct main0_in
        {
            float3 world_normal [[user(locn0)]];
            float3 world_pos [[user(locn1)]];
            float3 color [[user(locn2)]];
        };

        fragment main0_out main0(main0_in in [[stage_in]], constant fs_params& _17 [[buffer(1)]])
        {
            main0_out out = {};
            float3 _13 = fast::normalize(in.world_normal);
            float3 _23 = fast::normalize(_17.light_dir);
            out.frag_color = float4(((in.color * 0.20000000298023223876953125) + (in.color * fast::max(dot(_13, _23), 0.0))) + (float3(0.300000011920928955078125) * powr(fast::max(dot(_13, fast::normalize(_23 + fast::normalize(_17.view_pos - in.world_pos))), 0.0), 32.0)), 1.0);
            return out;
        }

    */
    static readonly byte[] fs_smooth_source_metal_macos = {
            ,    
};
    public static unsafe sg_shader_desc physics_demo_smooth_shader_desc(sg_backend backend) {
        if (backend == SG_BACKEND_METAL_MACOS) {
            sg_shader_desc desc = default(sg_shader_desc);
            desc.vertex_func.source = System.Text.Encoding.UTF8.GetString(vs_smooth_source_metal_macos);
            desc.vertex_func.entry = "main0";
            desc.fragment_func.source = System.Text.Encoding.UTF8.GetString(fs_smooth_source_metal_macos);
            desc.fragment_func.entry = "main0";
            desc.attrs[0].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[1].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[2].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[3].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[4].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[5].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[6].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.uniform_blocks[0].stage = SG_SHADERSTAGE_VERTEX;
            desc.uniform_blocks[0].layout = SG_UNIFORMLAYOUT_STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].msl_buffer_n = 0;
            desc.uniform_blocks[1].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.uniform_blocks[1].layout = SG_UNIFORMLAYOUT_STD140;
            desc.uniform_blocks[1].size = 32;
            desc.uniform_blocks[1].msl_buffer_n = 1;
            desc.label = "physics_demo_smooth_shader";
            return desc;
        }
        return default(sg_shader_desc);
    }
    public static int physics_demo_smooth_attr_slot(string attr_name) {
        if (attr_name == "position") {
            return 0;
        }
        if (attr_name == "normal") {
            return 1;
        }
        if (attr_name == "inst_model_0") {
            return 2;
        }
        if (attr_name == "inst_model_1") {
            return 3;
        }
        if (attr_name == "inst_model_2") {
            return 4;
        }
        if (attr_name == "inst_model_3") {
            return 5;
        }
        if (attr_name == "inst_color") {
            return 6;
        }
        return -1;
    }
    public static int physics_demo_smooth_texture_slot(string tex_name) {
        return -1;
    }
    public static int physics_demo_smooth_sampler_slot(string smp_name) {
        return -1;
    }
    public static int physics_demo_smooth_uniformblock_slot(string ub_name) {
        if (ub_name == "vs_params") {
            return 0;
        }
        if (ub_name == "fs_params") {
            return 1;
        }
        return -1;
    }
    public static int physics_demo_smooth_uniformblock_size(string ub_name) {
        if (ub_name == "vs_params") {
            return Marshal.SizeOf<vs_params_t>();
        }
        if (ub_name == "fs_params") {
            return Marshal.SizeOf<fs_params_t>();
        }
        return 0;
    }
    public static int physics_demo_smooth_uniform_offset(string ub_name, string u_name) {
        if (ub_name == "vs_params") {
            if (u_name == "vp") {
                return 0;
            }
        }
        if (ub_name == "fs_params") {
            if (u_name == "light_dir") {
                return 0;
            }
            if (u_name == "view_pos") {
                return 16;
            }
        }
        return -1;
    }
    public static sg_glsl_shader_uniform physics_demo_smooth_uniform_desc(string ub_name, string u_name) {
        sg_glsl_shader_uniform res = default;
        if (ub_name == "vs_params") {
            if (u_name == "vp") {
                res.type = SG_UNIFORMTYPE_MAT4;
                res.array_count = 0;
                res.glsl_name = "vp";
                return res;
            }
        }
        if (ub_name == "fs_params") {
            if (u_name == "light_dir") {
                res.type = SG_UNIFORMTYPE_FLOAT3;
                res.array_count = 0;
                res.glsl_name = "light_dir";
                return res;
            }
            if (u_name == "view_pos") {
                res.type = SG_UNIFORMTYPE_FLOAT3;
                res.array_count = 0;
                res.glsl_name = "view_pos";
                return res;
            }
        }
        return res;
    }
    public static int physics_demo_smooth_storagebuffer_slot(string sbuf_name) {
        return -1;
    }
    public static int physics_demo_smooth_storageimage_slot(string simg_name) {
        return -1;
    }
}

} // namescape 

