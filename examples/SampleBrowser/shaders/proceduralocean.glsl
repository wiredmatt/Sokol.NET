//------------------------------------------------------------------------------
//  Procedural Ocean shader based on afl_ext's ShaderToy example
...
vec3 getRay(vec2 fragCoord) {
    vec2 uv_coord = ((fragCoord.xy / iResolution.xy) * 2.0 - 1.0) * vec2(iResolution.x / iResolution.y, 1.0);
    vec3 proj = normalize(vec3(uv_coord.x, uv_coord.y, 1.5));
    if(iResolution.x < 600.0) {
        return proj;
    }
    // Fixed camera angle without mouse interaction
    return createRotationMatrixAxisAngle(vec3(0.0, -1.0, 0.0), 0.0) 
        * createRotationMatrixAxisAngle(vec3(1.0, 0.0, 0.0), 0.5 + 1.5 * (0.27 * 2.0 - 1.0))
        * proj;
}
...
void main() {
    ...
    if(ray.y >= 0.0) {
        ...
        frag_color = vec4(aces_tonemap(C * 2.0), 1.0);   
        return;
    }
