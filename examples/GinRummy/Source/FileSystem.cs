...
namespace Sokol
{
    ...
    public unsafe class FileSystem
    {
        ...
        private const int MAX_CONCURRENT_REQUESTS = 64; // Enough for bin + all textures in large models
        private const int NUM_CHANNELS = 2;             // Separate channels for different request types
        private const int NUM_LANES = 4;               // Lanes per channel (more throughput)
        ...
        private void StartRequest(FileLoadRequest request)
        {
            ...
            {
                // Pool exhausted — re-enqueue at front so it retries next Update() instead of failing permanently.
                // This can happen if many requests arrive between frames (e.g. bin + textures for a complex model).
