namespace TexasHoldem.Logic.GameMechanics
{
    ...
    public class TexasHoldemGame : ITexasHoldemGame
    {
        ...
        /// <summary>Index into SmallBlinds array for the opening blind level.</summary>
        public int InitialSmallBlindIndex { get; set; } = 0;

        /// <summary>When true, the blind level rises by 1 every BlindsHandPeriod hands.</summary>
        public bool EscalateBlinds { get; set; } = false;

        /// <summary>Number of hands between each blind level increase.</summary>
...
#if WEB
        public async Task<IPlayer> StartAsync(Func<Task>? afterEachHandAsync = null)
        {
            var playerNames = this.allPlayers.Select(x => x.Name).ToList().AsReadOnly();
            foreach (var player in this.allPlayers)
            {
            ...
            await this.PlayGameAsync(afterEachHandAsync);
        ...
        private async Task PlayGameAsync(Func<Task>? afterEachHandAsync)
        {
            ...
            while (this.allPlayers.WithMoney().Count() > 1)
            {
                ...
                int blindLevel = this.EscalateBlinds
                    ? Math.Min(this.InitialSmallBlindIndex + (this.HandsPlayed - 1) / this.BlindsHandPeriod, SmallBlinds.Length - 1)
                    : this.InitialSmallBlindIndex;
                var smallBlind = SmallBlinds[blindLevel];
                ...
                await ((HandLogic)hand).PlayAsync(afterEachHandAsync);
        ...
        private void PlayGame()
        {
            ...
            while (this.allPlayers.WithMoney().Count() > 1)
            {
                ...
                // Blind level: static or escalating based on configuration
                int blindLevel = this.EscalateBlinds
                    ? Math.Min(this.InitialSmallBlindIndex + (this.HandsPlayed - 1) / this.BlindsHandPeriod, SmallBlinds.Length - 1)
                    : this.InitialSmallBlindIndex;
                var smallBlind = SmallBlinds[blindLevel];
