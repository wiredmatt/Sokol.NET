...
#if WEB
        public async Task PlayAsync(Func<Task>? afterHandEndAsync = null)
        {
            foreach (var player in this.players)
            {
                var startHandContext = new StartHandContext(
            ...
            await this.PlayRoundAsync(GameRoundType.PreFlop, 0);

            if (this.players.Count(x => x.PlayerMoney.InHand) > 1)
                await this.PlayRoundAsync(GameRoundType.Flop, 3);

            if (this.players.Count(x => x.PlayerMoney.InHand) > 1)
                await this.PlayRoundAsync(GameRoundType.Turn, 1);

            if (this.players.Count(x => x.PlayerMoney.InHand) > 1)
                await this.PlayRoundAsync(GameRoundType.River, 1);
            ...
            // Allow the caller (PokerGame) to perform async operations after EndHand
            // (e.g. awaiting the "Continue" button press before the next hand starts).
            if (afterHandEndAsync != null)
                await afterHandEndAsync();
        }

        private async Task PlayRoundAsync(GameRoundType gameRoundType, int communityCardsCount)
        {
            ...
            await this.bettingLogic.BetAsync(gameRoundType);
